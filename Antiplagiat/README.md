...
**Ответственность:**  
Принимает файлы работ от студентов, сохраняет их (на диск / в S3 / в volume) и ведёт учёт в базе данных.
...
- `POST /analysis` — запуск анализа работы.
    - Вход: `workId` (и по желанию — настройки анализа).
    - Внутри:
...
Postman:
- В репозитории рекомендуется положить экспорт коллекции Postman (JSON) — если он есть, укажите путь. Если хотите, я могу сгенерировать пример коллекции.
...
- Сохранение файла в памяти целиком (при больших файлах) может потребовать много RAM. Для больших файлов стоит стримить/обрабатывать частями.
- Нет аутентификации/авторизации (MVP). В реальном проекте нужно защитить API (JWT, mTLS и т.п.).
